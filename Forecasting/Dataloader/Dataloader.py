...
class validation_dataset(Dataset):
    ...
    # ll, ul: lower and upper (bottom and top) boundaries delineating the retinal tissue ILM (upper) and BM (lower). Saved along the images in the dataset ? 
...
class train_dataset(Dataset):
    ...
    def pre_load_data(self):
        ...
        #img_pth='/'
        for k in range(0, sz):
            a=np.load(self.pth+self.full_nm_lst[k]+'.npz') #npz: compressed numpy file.
            img=a['img']
            ll=a['ll']
            ul=a['ul']
            del a
    ...
    def data_augmentation(self, img, nm, ll, ul):
        ...
        mn_slc=random.randint(0,2) # Here, selects the 3 consecutive B-scans
